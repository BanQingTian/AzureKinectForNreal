...
public class ZCoroutiner : System.Object
{
    ...
    /// 启动一个协程.
    /// </summary>
    public static Coroutine StartCoroutine(System.Action action, float delayTime = 0)
    {
        return StartCoroutine(CommCoroutine(action, delayTime));
    }

    private static IEnumerator CommCoroutine(System.Action action, float delayTime = 0)
    {
        if (delayTime == 0)
        {
            yield return null;
